        ...
        tokio::select! {
            Some(msg) = ws_receiver.next() => {
                ...
                match parsed_type {
                    ...
                    "play" => {
                        let mut board = [[false; BOARD_WIDTH]; BOARD_HEIGHT];
                        for (r_index, row) in parsed["board"].as_array().unwrap().iter().enumerate() {
                            for (c_index, col) in row.as_array().unwrap().iter().enumerate() {
                                board[r_index][c_index] = col.as_bool().unwrap();
                            }
                        }
                        bot.get_game_mut().board.set_arr(board);
