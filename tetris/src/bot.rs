...
impl Player for Bot {
    ...
    fn get_next_move(&mut self) -> CommandList {
        ...
        let (deep_moves, places, deep_scores) = self.move_placement_score(5, &self.weight.clone());
        ...
        for (moves, _, score) in izip!(deep_moves, places, deep_scores) {
            if score < min_score {
                min_score = score;
                action = moves;
                // p = place;
...
impl Bot {
    ...
    pub fn moves_to_placement(game: &mut Game, piece: &Piece) -> Result<CommandList, usize> {
    ...
    pub fn moves_to_placements(game: &mut Game, bag: &[Piece; 7]) -> Result<CommandList, usize> {
        let (moves, placements, _) = Bot::move_placement_score_1d(game, &Weights::default());
        for (m, p) in zip(moves, placements) {
            for piece in bag {
                if &p == piece {
    ...
    pub fn do_opener(&mut self) -> Result<CommandList, usize> {
        ...
        placement.moved(PointVector(
            0 - self.get_game().game_data.lines_cleared as i8,
            0,
        ));
    ...
    pub fn move_placement_score(
        ...
        let mut curr_placements: Vec<PlacementList> =
            temp_placements.into_iter().map(|x| vec![x]).collect();
        ...
        for curr_depth in 1..depth {
            ...
            if (curr_depth) % prune_depth == 0 {
                ...
                let combined_scores: Vec<Score> = curr_scores
                    .clone()
                    .into_iter()
                    .map(|(versus, board)| versus + board)
                    .collect();

                (curr_moves, curr_placements) = zip(curr_moves, curr_placements)
                    .enumerate()
                    .sorted_by(|&(i1, _), &(i2, _)| {
                        combined_scores[i1]
                            .partial_cmp(&combined_scores[i2])
                            .unwrap()
                    })
                    .take(n)
                    .map(|(_, p)| p)
                ...
                curr_scores
                    .sort_by(|(v1, b1), (v2, b2)| (v1 + b1).partial_cmp(&(v2 + b2)).unwrap());
                curr_scores.truncate(n);
            }

            //generating next_mps
            for (one_move, placements, (_, versus)) in izip!(
                curr_moves.clone(),
                curr_placements.clone(),
                curr_scores.clone()
            ) {
    ...
    fn score_game(mut game: Game, weights: &Weights, piece: &Piece) -> (Score, Score) {
        ...
        if false && (game.board.get_mino_count() % 2 == 0) {
            ...
            if parity.1 == false
                && !(usable_queue.contains(&1)
                    || usable_queue.contains(&5)
                    || usable_queue.contains(&6)
                    || !game.board.get_min_height() >= 2)
            {
                return penalty;
            }
        }
    ...
    fn score_versus(game_data: &GameData, weight: &Weights) -> Score {
        ...
        if t_spin {
            extra += weight.tspin_weight * game_data.last_cleared as f32;
        }
